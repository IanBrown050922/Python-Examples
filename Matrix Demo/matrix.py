class Matrix:
    ...
    '''
    _sub_pivot_zero_check (overloaded). This version of the method is used in calculating the matrix inverse, where row-operations are performed in parallel on two matrices: on the matrix whose inverse you're looking for and on the identity matrix.
    ...
    Return: True if every entry including and below the pivot is zero, otherwise False
    '''
    def _sub_pivot_zero_check(self, pivot: int, col: int, other: 'Matrix'):
        all_zeros = False
    ...
    '''
    _sub_pivot_eliminate (overloaded). This version of the method is used in calculating the matrix inverse, where row-operations are performed in parallel on two matrices: on the matrix whose inverse you're looking for and on the identity matrix.
    ...
    '''
    def _sub_pivot_eliminate(self, pivot: int, col: int, other: 'Matrix'):
        for i in range(pivot + 1, self._matrix.__len__()):
            if self._matrix[i][col] != 0:
                try:
                    scalar = -1 * (float(self._matrix[i][col]) / float(self._matrix[pivot][col]))
    ...
    '''
    _sup_pivot_eliminate (overloaded). This version of the method is used in calculating the matrix inverse, where row-operations are performed in parallel on two matrices: on the matrix whose inverse you're looking for and on the identity matrix.
    ...
    '''
    def ref(self, decimals: int, other: 'Matrix'):
        ref = Matrix(self.rows, self.cols, True)
        ref.set_matrix(self)
        ...
        if ref.rows == other.rows and ref.cols == other.cols:
            ...
            for i in range(max_pivots): # for every potential pivot column
                all_zeros = ref._sub_pivot_zero_check(pivot + 1, i, other)

                if not all_zeros:
                    pivot = pivot + 1
                    ref._sub_pivot_eliminate(pivot, i, other)
    ...
    '''
    def rref(self, decimals: int, other: 'Matrix'):
        rref, other = self.ref(decimals, other)
        max_pivots = min(rref.rows, rref.cols) # max possible pivot positions

        for i in range(max_pivots): # for each relevant column
            for j in range(rref.rows - 1, -1, -1): # for each entry bottom up
                if rref._matrix[j][i] != 0:
                    rref._sup_pivot_eliminate(j, i, other) ## HERE -- Done
                    break
    ...
    def inv(self, decimals: int) -> 'Matrix':
        return self.rref(decimals, Matrix.identity(self.rows))[1]